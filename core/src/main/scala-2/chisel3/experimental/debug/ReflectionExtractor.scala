...
import chisel3._
import chisel3.{KnownWidth, UnknownWidth}
...
object ReflectionExtractor {
  ...
  /**
   * Returns a string representation of a Chisel Data type name.
   */
  private def dataTypeName(data: Data): String = data match {
    case u: UInt   => s"UInt<${u.getWidth}>"
    case s: SInt   => s"SInt<${s.getWidth}>"
    case v: Vec[_] => s"${dataTypeName(v.sample_element)}[${v.length}]"
    case r: Record => r.className // MyBundle, AnonymousBundle
    case _ => data.getClass.getSimpleName
  }

  /**
   * Recursively extracts hierarchical fields from Chisel Data types.
   */
  private def extractDataHierarchy(data: Data, prefix: String = ""): Seq[FieldInfo] = {
    data match {
      case r: Record =>
        r.elements.toSeq.flatMap { case (name, elem) =>
          val fieldName = if (prefix.isEmpty) name else s"$prefix.$name"
          val childFields = extractDataHierarchy(elem, fieldName)
          FieldInfo(fieldName, dataTypeName(elem), serializeField(elem)) +: childFields
        }
      case v: Vec[_] =>
        v.headOption.map { elem =>
          val innerType = dataTypeName(v.sample_element)
          val fieldName = if (prefix.isEmpty) "_element" else s"$prefix._element"
          val childFields = extractDataHierarchy(elem, fieldName)
          Seq(FieldInfo(fieldName, innerType, s"${v.length} elements")) ++ childFields
        }.getOrElse(Seq.empty)
      case _ => Seq.empty // Ground types have no children
    }
  }
  ...
  def extract[T](obj: T): ClassDebugInfo = {
    ...
    // Extract Chisel Data hierarchy if applicable
    val dataFields = obj match {
      case d: Data => extractDataHierarchy(d)
      case _ => Seq.empty
    }

    ClassDebugInfo(className, fields ++ dataFields)
  }
  ...
  private def serializeField(value: Any): String = {
    value match {
      ...
      case _: Seq[_] =>
        ...
        case _ if value != null && (value.isInstanceOf[chisel3.Width] || value.isInstanceOf[KnownWidth] || value == UnknownWidth) =>
          value.toString // Handle Width specially - just return string representation
        case p: Product =>
          try {
            serializeProduct(value, forJson = false)
          } catch {
            case _: Throwable => s"<${value.getClass.getSimpleName}>"
          }
    ...
    if (value == null) {
      ...
      value match {
        ...
        case b: Boolean =>
          b.toString
        case _ if value != null && (value.isInstanceOf[chisel3.Width] || value.isInstanceOf[KnownWidth] || value == UnknownWidth) =>
          s"\"${value.toString}\"" // Handle Width specially
